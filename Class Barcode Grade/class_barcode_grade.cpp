...
bool OneDBarcodeGrading::EAN128(CImg* pImg, OneDBarcodeGrading::Grade &grade) {
	if (pImg == NULL)
		return false;
#ifdef CALC_TIME
	vector< pair<string, clock_t > > m_TIME_SET;

	m_TIME_SET.push_back(make_pair("Start:", clock()));
#endif
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("Trans CImg To BUFFER:", clock()));
#endif
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("Mapping:", clock()));
#endif
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("Difference:", clock()));
#endif

	Func_SmoothDifference(AMX, MaxOfAMX, 0.35);

#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("Smooth:", clock()));
#endif
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("Find Peaks:", clock()));
#endif
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("Find Farmost Distance Between Adjacent Peaks:", clock()));
#endif
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("Get GT:", clock()));
#endif
	...
	if (SC != 0) {
#ifdef CALC_TIME
		m_TIME_SET.push_back(make_pair("Scan Start:", clock()));
#endif
		vector<vector<unsigned long > > locDataSet;
		vector<vector<float > > locDataSet2;
#ifdef CALC_TIME
		Func_Scan(pImg, source, locDataSet, locDataSet2, GT, topmost, downmost, width, height, grade, m_TIME_SET);
#else
		Func_Scan(pImg, source, locDataSet, locDataSet2, GT, topmost, downmost, width, height, grade);
#endif

#ifdef CALC_TIME
		m_TIME_SET.push_back(make_pair("Scan End:", clock()));
#endif

		Func_Grading(grade);

#ifdef CALC_TIME
		m_TIME_SET.push_back(make_pair("Grading:", clock()));
#endif

		Func_Decodability(locDataSet2, OneDBarCodeType::EAN_128, grade);

#ifdef CALC_TIME
		m_TIME_SET.push_back(make_pair("Decodability:", clock()));
#endif
	}
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("End:", clock()));
#endif

#ifdef CALC_TIME
	for (vector< pair<string, clock_t > >::iterator p = m_TIME_SET.begin(); p != m_TIME_SET.end(); ++p) {
		cout << p->first << " " << p->second << endl;
	}
	cout << "-----\nSUM: " << (m_TIME_SET.end() - 1)->second - m_TIME_SET.begin()->second << endl;
	cout << endl;
#endif
...
//获得指定行号的像素值
bool OneDBarcodeGrading::_Func_GetSelectedRowPixels(unsigned char** img, vector<unsigned char> &rowSet, const unsigned long rowNum, const long width) {
	if (img == NULL) return false;
		...
		if (peakAndvalley.size() != 0) {
			for (vector<vector<unsigned char > >::iterator p = peakAndvalley.begin() + 1; p < peakAndvalley.end(); ++p) {
				if (p->size() == 0) {
					...
					while (p < peakAndvalley.end() && p->size() == 0)
						++p;
					if (p >= peakAndvalley.end())
						break;
...
bool OneDBarcodeGrading::Func_Scan(const CImg* pImg, unsigned char** img, vector<vector<unsigned long > > &locDataSet, vector<vector<float > > &locDataSet2, double GT, const unsigned int topmost, const unsigned int downmost, const long width, const long height, OneDBarcodeGrading::Grade &grade) {
	...
	unsigned long d = downmost - topmost;
	double distance;
	if (d >= 10) {
		//扫描线之间的距离
		distance = (downmost - topmost)*1.0 / 10.0;
	}
	else {
		distance = 1;
	}
...
//根据topmost和downmost两条界限确定的条码范围，进行分割，建立多条扫描线进行横向扫描
bool OneDBarcodeGrading::Func_Scan(const CImg* pImg, unsigned char** img, vector<vector<unsigned long > > &locDataSet, vector<vector<float > > &locDataSet2, double GT, const unsigned int topmost, const unsigned int downmost, const long width, const long height, OneDBarcodeGrading::Grade &grade, vector<pair<string, clock_t > > &T) {
	if (img == NULL) return false;
	if (width == 0 || height == 0) return false;
	if (topmost<0 || downmost>height) return false;
	//所有扫描线的像素数据集合
	...
	for (long y = topmost; y <= downmost; y += distance) {
		...
		T.push_back(make_pair("Scan - GetPixels:", clock()));
		...
		T.push_back(make_pair("Scan - Split:", clock()));
	...
	T.push_back(make_pair("Scan - EC_ERN:", clock()));
...
bool TwoDBarcodeGrading::Grading(CImg* pImg, const unsigned int error_correction_capacity, TwoDBarCodeType type) {
	...
	case TwoDBarCodeType::DataMatrix: {
		m_TIME_SET.clear();
		bool rt = false;
		if (pImg->GetWidthPixel()*pImg->GetHeight() >= THRESHOLD_DIMENSIONS) {
			CImg* pMidImg = NULL;
			rt = Func_DataMatrixClipping(pImg, pMidImg);
			rt = DataMatrixGrading(pMidImg, error_correction_capacity, TwoDBarcodeGrade);
			InitFlag = true;
			return rt;
		}
		else {
			rt = DataMatrixGrading(pImg, error_correction_capacity, TwoDBarcodeGrade);
			InitFlag = true;
			return rt;
		}
	}
...
bool TwoDBarcodeGrading::Func_DataMatrixClipping(CImg* pImg, CImg* &pDstImg) {
	if (pImg == NULL)
		return false;
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("Scaling Start:", clock()));
#endif

	Gray_Image_Processing action;
	CImg* pMidImg = NULL;

	action.Scaling(pImg, pMidImg, DMTX_SCALING_RATE, Gray_Image_Processing::SCALING_METHOD::SC_NONE);
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("Scaling Over:", clock()));
#endif
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("libdmtx Get Region Over:", clock()));
#endif
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("clip region before rotate:", clock()));
#endif
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("Get Angle and rotate:", clock()));
#endif
	...
	if (width_a >= 0 && height_a >= 0) {
		...
		pDstImg = create_image(rcimg);
	}
	else {
		pDstImg = create_image(rimg);
	}

#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("Clipping Over:", clock()));
#endif
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("Clear:", clock()));
#endif
}
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("Grading Start:", clock()));
#endif
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("Calc SC:", clock()));
#endif
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("Contrast Control:", clock()));
#endif
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("Sobel:", clock()));
#endif
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("Sobel Gradient:", clock()));
#endif
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("Gradient Mapping:", clock()));
#endif
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("Gauss:", clock()));
#endif
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("Get Peaks:", clock()));
#endif
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("Compare:", clock()));
#endif
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("Erase Surround Dark:", clock()));
#endif
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("Gray Value AVG:", clock()));
#endif

	rt = __Func_DrawLine(source, Xmaxima, Ymaxima, width, height, (unsigned char)GT);
	if (rt == false)
		return false;
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("Calc All Modulation:", clock()));
#endif

	if (rt != false) {
		grade.MOD = _Func_MOD_T7A(DataAreaMODInfo, MODGradingTable, error_correction_capacity);
#ifdef CALC_TIME
		m_TIME_SET.push_back(make_pair("Modulation Table:", clock()));
#endif
	}
	else {
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("AN:", clock()));
#endif
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("GN:", clock()));
#endif
...
#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("FPD:", clock()));
#endif

	Func_CalculateFPDAverageGrade(FPD_Grades, grade);

#ifdef CALC_TIME
	m_TIME_SET.push_back(make_pair("FPD AVG Grade:", clock()));
#endif
...
bool TwoDBarcodeGrading::Func_CalculateFPDAverageGrade(TwoDBarcodeGrading::FPD_G FPD_grades, TwoDBarcodeGrading::Grade &grade) {
...
//根据得到的波峰点位绘制网格
bool TwoDBarcodeGrading::__Func_DrawLine(unsigned char** img, vector<long > Xmaxima, vector<long > Ymaxima, const long width, const long height, const unsigned char color) {
	if (img == NULL || Xmaxima.size() == 0 || Ymaxima.size() == 0)
		return false;
	unsigned char** preview = new unsigned char*[height];
	for (unsigned long i = 0; i < height; ++i) {
