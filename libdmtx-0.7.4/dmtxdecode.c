...
/**
 * \brief  将匹配的DM区域转换为解码消息 Convert fitted Data Matrix region into a decoded message
 * \param  dec
 * \param  reg
 * \param  fix
...
/**
 * \brief  根据模块颜色为标准判断码字的值并填充数组 Populate array with codeword values based on module colors
 * \param  msg
 * \param  img
 * \param  reg
 * \return DmtxPass | DmtxFail
...
PopulateArrayFromMatrix(DmtxDecode *dec, DmtxRegion *reg, DmtxMessage *msg)
{
   ...
   int tally[24][24]; /* 足够以映射最大的单个区域 */ /* Large enough to map largest single region */
