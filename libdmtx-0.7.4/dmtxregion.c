...
/**
 * \brief  找到下一个条码区域 Find next barcode region
 * \param  dec 指向DmtxDecode信息结构体的指针 Pointer to DmtxDecode information struct
 * \param  timeout 指向DmtxTime结构体的指针 Pointer to timeout time (NULL if none)
 * \return 检测到的区域(如果能找到) Detected region (if found)
 */
extern DmtxRegion *
